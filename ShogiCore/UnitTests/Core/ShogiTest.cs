...
namespace ShogiCore.UnitTests.ShogiCore
{
  ...
  public class ShogiTest
  {
    ...
    private static readonly ReadOnlyDictionary<Position, IPieceType> _initialPosition = new[]
      {
        "1a", "香", "9a", "香", "1i", "香", "9i", "香",
        "2a", "桂", "8a", "桂", "2i", "桂", "8i", "桂",
        "3a", "銀", "7a", "銀", "3i", "銀", "7i", "銀",
    ...
    public void InitialPositionTest()
    {
      ...
      foreach (var pair in _initialPosition)
        Assert.AreEqual(pair.Value, _board.GetPieceAt(pair.Key).PieceType);
      //    _________________________________
    ...
    public void CellBindabilityTest()
    {
      ...
      var handler = new Action<Position, Cell>(
        ...
        {
          counter++;
          Assert.AreEqual(_initialPosition[cell], placeHolder.Piece.PieceType);
        });
